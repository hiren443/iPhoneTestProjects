...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex==0)
    {
		...
		NSLog(@"%@",strName);
		//if (strName.length==0 || [strName hasPrefix:@"(null)"]) {
//			strName=@"Untitled Video";
//		}
		//strName=@"CS Link.txt";
		NSFileManager *fileManager = [[NSFileManager alloc] init];
		NSArray *paths2 = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
		NSString *directPath = [paths2 objectAtIndex:0];
		
		
		NSArray *documents = [fileManager contentsOfDirectoryAtPath:directPath error:nil];
		NSString *tempStr= [NSString stringWithFormat:@"%@",strName];
		int countOfFiles=0;
		int terminator=1;
		int nameCounter=0;
			...
			while (terminator) {
				...
				for (NSString *str in documents)	{
					...
					if ([str isEqualToString:tempStr]) {
						if (tempStr.length>1) {
							tempStr=[tempStr substringWithRange:NSMakeRange(0,tempStr.length-1)];
							NSLog(@"tmpstr%@",tempStr);
						}
						
						
						tempStr=[NSString stringWithFormat:@"%@%i",tempStr,++nameCounter];
						countOfFiles=0;
						break;
					}
					if (countOfFiles == [documents count]) {
						terminator=0;
						break;
					}
					countOfFiles ++;
