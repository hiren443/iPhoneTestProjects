...
static NSString *kAppId= @"145062165564860"; // externalize it!
...
- (void)viewDidLoad {
	...
	statusLabel=[[UILabel alloc]initWithFrame:CGRectMake(100, 70, 200, 100)];
	loginButton=[UIButton buttonWithType:UIButtonTypeRoundedRect] ;
	loginButton.frame=CGRectMake(100, 160, 120, 32);
	[loginButton setTitle:@"View MyFriends" forState:UIControlStateNormal];
...
-(void)login{
	if ([facebook isSessionValid]) {
		...
		[facebook authorize:_permissions delegate:self localAppId:kAppId];
	}
...
-(void) fbDidLogout
{
	...
	myTableView.hidden = TRUE;
	statusLabel.hidden = TRUE;
	statusLabel.text=@"Pls login first";
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
     if([arrayOfFriendsData count]==0) return 0;// check for some error cases dude!!!
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	textLabel.frame=CGRectMake(100,20, 300, 50);
	textLabel.text=name;
